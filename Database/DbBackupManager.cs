    ...
    public class DbBackupManager
    {
        ...
        public string CreateBackup()
        {
            try
            {
                ...
                string sqlServerBackupPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), 
                                                       "Microsoft SQL Server", "Backup", backupFileName);
                
                
                string sqlServerBackupDir = Path.GetDirectoryName(sqlServerBackupPath);
                if (!Directory.Exists(sqlServerBackupDir))
                {
                    Directory.CreateDirectory(sqlServerBackupDir);
                }
                
                
                string query = $@"BACKUP DATABASE [Dispanserizatsia] TO DISK = N'{sqlServerBackupPath}' 
                ...
                string userBackupPath = Path.Combine(backupFolder, backupFileName);
                File.Copy(sqlServerBackupPath, userBackupPath, true);
                
                // Удаляем временный файл
                try
                {
                    File.Delete(sqlServerBackupPath);
                }
                catch
                {
